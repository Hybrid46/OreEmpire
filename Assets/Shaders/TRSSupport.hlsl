...
inline void SetUnityMatrices(uint instanceID, inout float4x4 objectToWorld, inout float4x4 worldToObject)
{
    ...
    objectToWorld = mul(objectToWorld, _DrawData[instanceID].TRS);
...
void passthroughVec3_float(in float3 Local,in float InstanceID, out float3 Transformed)
{
    #if UNITY_ANY_INSTANCING_ENABLED
        SetUnityMatrices(InstanceID, unity_ObjectToWorld, unity_WorldToObject);
    #endif
