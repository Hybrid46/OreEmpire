...
public static class UVMapper
{
    ...
    // and map to them.
    public static void BoxUV(Mesh mesh, Transform tform, Color[] Colors)
    {
        // Matrix 
        ...
        List<Vector3> newUVs = new List<Vector3>(verts.Length);
        ...
        for (int s = 0; s < mesh.subMeshCount; s++)
        {
            ...
            for (int t = 0; t < tris.Length; t += 3)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        // Compute UV
                        Vector2 gbu = GetBoxUV(worldVerts[v], boxDir);
                        Vector3 vertexUV = new Vector3(gbu.x,gbu.y,Colors[v].r);
